...
/**
 * A premium 3D card (GSAP CSS-3D). The OUTER element owns the `perspective`;
 * the INNER element is `preserve-3d` and is what actually tilts/scales (so its
 * z-translated child layers read with real depth). On a fine pointer the card
 * tilts toward the cursor with a moving sheen; on touch / reduced-motion it
 * stays calm (a slow idle float on touch).
 *
 * The inner DOM node is handed back via `cardRef` so a variant's choreography
 * (e.g. `flyToFill`) can fly this exact card toward the viewer and open it onto
 * the next stage.
 *
 * Decoupled `core/` version — depends only on `gsap` + the reduced-motion
 * helper, NOT on the old `three/` provider/context.
 *
 * @param {{
 *   children: React.ReactNode,
 *   onClick?: () => void,
 *   interactive?: boolean,   // pointer tilt + sheen (turn off for the form stage)
 *   ariaLabel?: string,
 *   radius?: number,         // corner radius (px)
 *   tilt?: number,           // max tilt degrees
 *   cardRef?: (el: HTMLElement | null) => void, // receives the INNER node
